...
Open OSCAR Server includes a lightweight browser client for the Web AIM API. The WebAPI server serves the client from static files on disk (default: `server/webapi/webclient`). Start the server with the WebAPI enabled, create a Web API key, then open `http://localhost:9000/client/`.
